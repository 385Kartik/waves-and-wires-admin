...
import {
  ...
  CheckCircle, XCircle, Eye, Package, AlertCircle, Link2, RotateCcw,
} from 'lucide-react';
...
export default function AdminOrders() {
  ...
  async function syncFromShiprocket() {
    ...
    try {
      const res  = await fetch('/api/shiprocket', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body:   JSON.stringify({ action: 'get_order_details', payload: { shiprocket_order_id: selected.shiprocket_order_id } }),
      });
      ...
      if (awb) {
        const update = { awb_code: awb, courier_name: courier, shiprocket_shipment_id: shipmentId ?? selected.shiprocket_shipment_id };
        await supabase.from('orders').update(update).eq('id', selected.id);
        setSelected(p => p ? { ...p, ...update } : p);
        toast.success('AWB synced!');
        load();
      } else {
        toast.info('No AWB yet. Try after courier assignment.');
      }
    } catch (err: any) {
      toast.error('Sync failed: ' + err.message);
    } finally { setSrSyncing(false); }
  }
  ...
  async function fetchSrTracking(awb?: string) {
    ...
    } catch { toast.error('Tracking fetch failed'); }
    finally { setSrTrackLoading(false); }
  }
  ...
  const updateStatus = async (id: string, newStatus: string) => {
    ...
    try {
      if (newStatus === 'cancelled' && selected.shiprocket_order_id) {
        ...
        const srPayload = selected.awb_code ? { awb_code: selected.awb_code } : { shiprocket_order_id: selected.shiprocket_order_id };
  ...
  async function processRefund() {
    ...
    // COD orders can't be refunded via PhonePe
    if (selected.payment_method === 'cod') {
      toast.error('COD orders cannot be refunded via PhonePe. Mark as refunded manually.');
      return;
    }

    // payment_ref = actual PhonePe transaction ID
    if (!selected.payment_ref) {
      toast.error('Payment reference not found. Cannot process refund automatically.');
      return;
    }

    if (!window.confirm(`Refund ₹${selected.total.toLocaleString('en-IN')} to customer via PhonePe?\n\nThis is irreversible.`)) return;

    setRefunding(true);
    try {
      // Step 1: PhonePe se refund initiate karo
      const res    = await fetch('/api/phonepe-refund', {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify({
          payment_ref:  selected.payment_ref,   // WW-1234-timestamp (actual PhonePe txn ID)
      ...
      if (!result.success) {
        toast.error(result.error ?? 'PhonePe refund failed');
        return;
      }

      // Step 2: DB update karo
      ...
      setSelected(p => p ? { ...p, status: 'refunded', payment_status: 'refunded' } : p);
      setManualStatus('refunded');
      toast.success(`Refund of ₹${selected.total.toLocaleString('en-IN')} initiated successfully!`);
      load();
    } catch (err: any) {
  ...
  const canRefund = selected?.status === 'cancelled' && selected?.payment_status === 'paid' && selected?.payment_method !== 'cod';
    ...
    <div className="p-6 space-y-5">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        {searchInput !== searchQuery && (
          <button onClick={() => setSearchQuery(searchInput)} className="rounded-xl bg-zinc-900 text-white px-4 py-2 text-sm font-semibold">Search</button>
        )}
        {searchQuery && (
          <button onClick={() => { setSearchInput(''); setSearchQuery(''); }} className="flex items-center gap-1.5 text-sm text-zinc-500 hover:text-zinc-700">
        ...
        <div className="flex gap-1 flex-wrap">
          {STATUSES.map(s => (
            <button key={s} onClick={() => setStatusFilter(s)}
              className={`px-3 py-1.5 rounded-xl text-xs font-semibold capitalize transition-all ${statusFilter === s ? 'bg-zinc-900 text-white' : 'bg-white border border-zinc-200 text-zinc-600 hover:bg-zinc-50'}`}>
              {s}
            </button>
          ))}
        </div>
      ...
      <div className="bg-white rounded-2xl border border-zinc-200/80 overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center"><Package className="mx-auto h-8 w-8 text-zinc-300 mb-2" /><p className="text-sm text-zinc-400">No orders found</p></div>
        ) : (
              ...
              <tbody>
                {orders.map(o => (
                  <tr key={o.id} className="border-b border-zinc-50 hover:bg-zinc-50/50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-[10px] font-bold px-2 py-1 rounded-full uppercase border ${STATUS_COLORS[o.status] ?? ''}`}>{o.status}</span>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${PAYMENT_COLORS[o.payment_status] ?? ''}`}>{o.payment_status}</span>
                    </td>
                    ...
                    <td className="px-4 py-3 text-zinc-400 text-[11px] whitespace-nowrap">{format(new Date(o.created_at), 'dd MMM, h:mm a')}</td>
                    <td className="px-4 py-3 text-right">
                      <button onClick={() => openDetail(o)} className="p-1.5 hover:bg-zinc-100 rounded-lg transition-colors">
        ...
        {pages > 1 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-zinc-100">
            <p className="text-xs text-zinc-400">Page {page + 1} of {pages} · {totalCount} orders</p>
            <div className="flex gap-1">
              <button onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0} className="p-1.5 rounded-lg border border-zinc-200 hover:bg-zinc-50 disabled:opacity-40 transition-all"><ChevronLeft className="h-4 w-4" /></button>
              <button onClick={() => setPage(p => Math.min(pages - 1, p + 1))} disabled={page >= pages - 1} className="p-1.5 rounded-lg border border-zinc-200 hover:bg-zinc-50 disabled:opacity-40 transition-all"><ChevronRight className="h-4 w-4" /></button>
            </div>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex">
          ...
          <div className="w-full max-w-lg bg-white h-full overflow-y-auto shadow-2xl flex flex-col">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-100 sticky top-0 bg-white z-10">
              ...
              <button onClick={() => setSelected(null)} className="p-2 hover:bg-zinc-100 rounded-xl transition-colors"><X className="h-4 w-4" /></button>
            </div>
            ...
            <div className="p-6 space-y-6 flex-1">
              ...
              {/* ── PhonePe Refund ───────────────────────────────────────── */}
              {canRefund && (
                <section className="border border-red-200 rounded-2xl overflow-hidden bg-red-50">
                  <div className="px-4 py-3 border-b border-red-200 flex items-center gap-2">
                    <RotateCcw className="h-4 w-4 text-red-600" />
                    <span className="text-xs font-bold text-red-700 uppercase tracking-wide">Process Refund via PhonePe</span>
                  </div>
                  <div className="p-4 space-y-3">
                    <div className="text-sm text-red-700 space-y-1">
                      <p>Amount: <span className="font-black">{INR(selected.total)}</span></p>
                      <p className="text-xs text-red-600">
                        {selected.payment_ref
                          ? `Txn ID: ${selected.payment_ref}`
                          : '⚠️ No payment reference found — refund may fail'}
                      </p>
                    </div>
                    <button onClick={processRefund} disabled={refunding || !selected.payment_ref}
                      className="w-full bg-red-600 text-white py-2.5 rounded-xl text-sm font-bold flex justify-center items-center gap-2 hover:bg-red-700 disabled:opacity-50 transition-all">
                      {refunding
                        ? <><span className="h-4 w-4 rounded-full border-2 border-white/30 border-t-white animate-spin" />Processing Refund…</>
                        : <><RotateCcw className="h-4 w-4" />Refund {INR(selected.total)} via PhonePe</>}
                    </button>
                    <p className="text-[11px] text-red-500 text-center">⚠️ This will immediately initiate refund. Irreversible.</p>
                  </div>
                </section>
              )}

              {/* COD cancelled - manual refund note */}
              {selected.status === 'cancelled' && selected.payment_method === 'cod' && (
                <section className="border border-zinc-200 rounded-2xl overflow-hidden">
                  ...
                  <div className="p-4 space-y-3">
                    <p className="text-xs text-zinc-500">COD order — no online refund applicable.</p>
                    <button onClick={() => updateStatus(selected.id, 'refunded')} disabled={!!updatingId}
              ...
              <section className="border border-zinc-200 rounded-2xl overflow-hidden">
                <div className="bg-zinc-50 px-4 py-3 border-b border-zinc-200 flex items-center justify-between">
                  ...
                  {selected.shiprocket_order_id && (
                    <span className="text-[10px] bg-emerald-50 text-emerald-700 border border-emerald-200 px-2 py-0.5 rounded-full font-bold">SR #{selected.shiprocket_order_id}</span>
                  )}
                </div>
                <div className="p-4 space-y-3">
                  {selected.status === 'cancelled' ? (
                    ...
                    <div className="space-y-2">
                      <p className="text-xs text-zinc-500">Not pushed to Shiprocket yet.</p>
                      <button onClick={pushToShiprocket} disabled={srPushing}
                        ...
                        {srPushing ? <><span className="h-4 w-4 rounded-full border-2 border-white/30 border-t-white animate-spin" />Pushing…</> : <><Truck className="h-4 w-4" /> Push to Shiprocket</>}
                      </button>
                    </div>
                  ) : !selected.awb_code ? (
                    <div className="space-y-3">
                      <div className="flex items-center gap-2"><AlertCircle className="h-4 w-4 text-amber-500 shrink-0" /><p className="text-xs text-zinc-600">Pushed. Awaiting courier assignment.</p></div>
                      <button onClick={syncFromShiprocket} disabled={srSyncing}
                        ...
                        {srSyncing ? <><span className="h-3.5 w-3.5 rounded-full border-2 border-zinc-300 border-t-zinc-700 animate-spin" />Syncing…</> : <><RefreshCw className="h-3.5 w-3.5" /> Sync AWB</>}
                      </button>
                    </div>
                  ) : (
                    <div className="space-y-3">
                      ...
                      <button onClick={() => fetchSrTracking()} disabled={srTrackLoading}
                        className="w-full border border-zinc-300 text-zinc-700 py-2 rounded-xl text-sm font-semibold flex justify-center items-center gap-2 hover:bg-zinc-50 disabled:opacity-50 transition-all">
                        {srTrackLoading ? <><span className="h-3.5 w-3.5 rounded-full border-2 border-zinc-300 border-t-zinc-700 animate-spin" />Loading…</> : <><RefreshCw className="h-3.5 w-3.5" /> Refresh Tracking</>}
                      </button>
                      {srTracking && (
                        <div className="bg-zinc-50 rounded-xl p-3 space-y-1 text-xs">
                          <p className="font-bold text-zinc-800">{srTracking.shipment_status ?? srTracking.current_status ?? 'In Transit'}</p>
                          {srTracking.etd && <p className="text-zinc-500">ETA: {srTracking.etd}</p>}
                          {srTracking.awb_track_url && (
                            <a href={srTracking.awb_track_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-blue-600 font-semibold hover:underline">
              ...
              <section className="border border-zinc-200 rounded-2xl overflow-hidden">
                ...
                <div className="p-4 space-y-3">
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => updateStatus(selected.id, manualStatus)} disabled={!!updatingId || manualStatus === selected.status}
                  ...
                  <p className="text-[11px] text-zinc-400 flex items-center gap-1">
                    ...
                    Manual override — Shiprocket webhook may revert.
              ...
              {/* ── Customer ─────────────────────────────────────────────── */}
              <section className="border border-zinc-200 rounded-2xl overflow-hidden">
                <div className="bg-zinc-50 px-4 py-3 border-b border-zinc-200"><span className="text-xs font-bold text-zinc-700 uppercase tracking-wide">Customer</span></div>
              ...
              {/* ── Payment ──────────────────────────────────────────────── */}
              <section className="border border-zinc-200 rounded-2xl overflow-hidden">
                <div className="bg-zinc-50 px-4 py-3 border-b border-zinc-200"><span className="text-xs font-bold text-zinc-700 uppercase tracking-wide">Payment</span></div>
                <div className="p-4 text-sm space-y-1.5">
                  <div className="flex justify-between text-zinc-600"><span>Method</span><span className="font-semibold text-zinc-800 uppercase">{selected.payment_method}</span></div>
                  <div className="flex justify-between text-zinc-600">
                    <span>Status</span>
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${PAYMENT_COLORS[selected.payment_status] ?? ''}`}>{selected.payment_status}</span>
                  </div>
                  {selected.payment_ref && (
                    <div className="flex justify-between text-zinc-600"><span>Txn ID</span><span className="font-mono text-xs text-zinc-500">{selected.payment_ref}</span></div>
                  )}
                  {selected.coupon_code && (
                    <div className="flex justify-between text-zinc-600"><span>Coupon</span><span className="font-mono font-bold text-green-700">{selected.coupon_code}</span></div>
                  )}
                  <div className="border-t border-zinc-100 pt-1.5 space-y-1">
                    ...
                    {selected.discount > 0 && <div className="flex justify-between text-green-600 text-xs"><span>Discount</span><span>−{INR(selected.discount)}</span></div>}
                    <div className="flex justify-between text-zinc-500 text-xs"><span>Shipping</span><span>{selected.shipping > 0 ? INR(selected.shipping) : 'Free'}</span></div>
                    <div className="flex justify-between text-zinc-500 text-xs"><span>Tax</span><span>{INR(selected.tax)}</span></div>
                    <div className="flex justify-between font-bold text-zinc-900 text-sm pt-1 border-t border-zinc-100"><span>Total</span><span>{INR(selected.total)}</span></div>
                  </div>
                </div>
              </section>
              ...
              <section className="border border-zinc-200 rounded-2xl overflow-hidden">
                <div className="bg-zinc-50 px-4 py-3 border-b border-zinc-200">
                  <span className="text-xs font-bold text-zinc-700 uppercase tracking-wide">Items ({selected.items?.length ?? 0})</span>
                </div>
                <div className="divide-y divide-zinc-50">
                  {(selected.items ?? []).map((item: OrderItem) => (
                    <div key={item.id} className="flex items-center gap-3 px-4 py-3">
                      {item.product_image
                        ? <img src={item.product_image} alt={item.product_name} className="h-10 w-10 rounded-xl object-cover border border-zinc-100 shrink-0" />
                        : <div className="h-10 w-10 rounded-xl bg-zinc-100 flex items-center justify-center shrink-0"><Package className="h-4 w-4 text-zinc-400" /></div>}
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold text-zinc-800 truncate">{item.product_name}</p>
                        <p className="text-xs text-zinc-400">SKU: {item.product_sku ?? '—'} · Qty: {item.quantity} × {INR(item.price)}</p>
                      </div>
              ...
              <div className="bg-zinc-50 border border-zinc-200 rounded-xl p-3 text-center">
                <p className="text-[11px] text-zinc-400 flex items-center justify-center gap-1.5">
                  ...
                  Statuses auto-update via Shiprocket webhook.
                </p>
              </div>
            </div>
          </div>
