...
interface RefundRequest {
  ...
  order_number: string; total: number; payment_method: string;
...
export default function AdminRefunds() {
  ...
  const load = useCallback(async () => {
    ...
    const { data, error } = await supabase
      ...
      .select('id, order_id, reason, status, admin_note, created_at, orders(order_number, total, shipping_address, payment_method)')
  ...
  async function updateStatus(id: string, newStatus: string, orderId: string) {
    ...
    try {
      ...
      // - completed → order status='refunded', payment_status='refunded'
      // - rejected  → order status back to 'delivered' (refund denied)
      // - approved  → no order status change yet (waiting for actual refund)
            ...
            <tbody className="divide-y divide-zinc-50">
              {loading ? [...Array(4)].map((_,i) => (
                ...
                <tr key={r.id} className="hover:bg-zinc-50/70 transition-colors group">
                  ...
                  <td className="px-4 py-3.5 text-xs text-zinc-500 capitalize">{r.payment_method === 'cod' ? 'COD' : 'Online'}</td>
          ...
          <div className="w-full max-w-md bg-white h-full overflow-y-auto shadow-2xl flex flex-col">
            ...
            <div className="flex-1 p-6 space-y-5">
              ...
              <div className="bg-zinc-50 rounded-2xl p-4 grid grid-cols-2 gap-3 text-sm">
                <div><p className="text-[10px] text-zinc-400 font-semibold uppercase tracking-wider mb-1">Customer</p><p className="font-semibold text-zinc-900">{selected.customer_name}</p></div>
                <div><p className="text-[10px] text-zinc-400 font-semibold uppercase tracking-wider mb-1">Phone</p><p className="font-semibold text-zinc-900">{selected.customer_phone}</p></div>
                <div><p className="text-[10px] text-zinc-400 font-semibold uppercase tracking-wider mb-1">Payment</p><p className="font-semibold text-zinc-900 capitalize">{selected.payment_method === 'cod' ? 'Cash on Delivery' : 'Online (Razorpay)'}</p></div>
                <div><p className="text-[10px] text-zinc-400 font-semibold uppercase tracking-wider mb-1">Amount</p><p className="font-bold text-zinc-900">{INR(selected.total)}</p></div>
              </div>

              <div>
                <p className="text-xs font-bold text-zinc-700 uppercase tracking-wider mb-2">Customer's Reason</p>
                <div className="bg-zinc-50 border border-zinc-200 rounded-2xl p-4 text-sm text-zinc-600 leading-relaxed">{selected.reason}</div>
              </div>
              ...
              {selected.status === 'pending' && (
                <>
                  ...
                  <div className="flex items-start gap-2 rounded-xl bg-zinc-50 border border-zinc-200 p-3 text-xs text-zinc-500">
                    ...
                    <span><strong>Reject</strong> → order goes back to "delivered". <strong>Approve</strong> → mark as Completed once you process the refund.</span>
                  </div>
                </>
              )}
              ...
              {selected.status === 'approved' && (
                <>
                  <div className="rounded-xl bg-blue-50 border border-blue-200 p-4 text-sm text-blue-700">
                    <strong>Approved.</strong> Once you've sent the refund to the customer, mark as Completed below.
                  </div>
                  <div>
                    <label className="text-xs font-bold text-zinc-700 uppercase tracking-wider mb-1.5 block">Admin Note (optional)</label>
              ...
              {selected.admin_note && !['pending'].includes(selected.status) && (
                <div>
                  <p className="text-xs font-bold text-zinc-700 uppercase tracking-wider mb-2">Admin Note</p>
                  <div className="bg-zinc-50 border border-zinc-200 rounded-2xl p-4 text-sm text-zinc-600">{selected.admin_note}</div>
                </div>
              )}
            </div>
          </div>
