...
import { TrendingUp, ShoppingCart, Users, Package, ArrowUp, ArrowDown, AlertTriangle, Clock, MoreHorizontal } from 'lucide-react';
...
function StatCard({ label, value, sub, icon: Icon, color, trend }:{ label:string; value:string; sub:string; icon:any; color:string; trend?:number }) {
  return (
    <div className="bg-white rounded-2xl border border-zinc-200/80 p-6 hover:shadow-md hover:border-zinc-300 transition-all duration-200">
  ...
  useEffect(() => {
    async function load() {
      const [s, o, c] = await Promise.all([
        ...
        supabase.from('orders').select('id,order_number,status,total,created_at,profiles:user_id(full_name)').order('created_at',{ascending:false}).limit(7),
        supabase.from('admin_revenue_by_day').select('*').limit(30),
      ]);
      if (s.data) setStats(s.data as Stats);
      if (o.data) setOrders(o.data.map((x:any)=>({...x,customer_name:x.profiles?.full_name??'Guest'})));
    ...
    <div className="p-6 space-y-6 max-w-[1400px]">
      ...
      {(stats?.low_stock_count??0)>0 && (
        <div className="flex items-center gap-3 rounded-xl bg-amber-50 border border-amber-200 px-4 py-3">
          ...
          <p className="text-sm text-amber-800"><strong>{stats?.low_stock_count}</strong> products are running low on stock.</p>
          <button onClick={()=>navigate('/dashboard/inventory')} className="ml-auto text-xs font-semibold text-amber-700 hover:text-amber-900 underline underline-offset-2">View →</button>
        </div>
      )}
      ...
      {(stats?.pending_refunds??0)>0 && (
        <div className="flex items-center gap-3 rounded-xl bg-red-50 border border-red-200 px-4 py-3">
          ...
          <button onClick={()=>navigate('/dashboard/refunds')} className="ml-auto text-xs font-semibold text-red-700 hover:text-red-900 underline underline-offset-2">Review →</button>
        </div>
      )}
      ...
      {(stats?.unread_messages??0)>0 && (
        <div className="flex items-center gap-3 rounded-xl bg-blue-50 border border-blue-200 px-4 py-3">
          ...
          <p className="text-sm text-blue-800"><strong>{stats?.unread_messages}</strong> unread message{(stats?.unread_messages??0)>1?'s':''} from customers.</p>
          <button onClick={()=>navigate('/dashboard/messages')} className="ml-auto text-xs font-semibold text-blue-700 hover:text-blue-900 underline underline-offset-2">View →</button>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-5 gap-6">
        {/* Revenue chart */}
        <div className="xl:col-span-3 bg-white rounded-2xl border border-zinc-200/80 p-6">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={chart} margin={{top:0,right:0,left:-20,bottom:0}}>
              ...
              <Tooltip contentStyle={{background:'#18181b',border:'1px solid #27272a',borderRadius:12,fontSize:12,color:'#fff'}} labelFormatter={l=>format(new Date(l),'PPP')} formatter={(v:number)=>[INR(v),'Revenue']}/>
        ...
        {/* Recent orders */}
        <div className="xl:col-span-2 bg-white rounded-2xl border border-zinc-200/80 p-6">
          ...
          <div className="space-y-3">
            {orders.map(o=>(
              <div key={o.id} onClick={()=>navigate('/dashboard/orders')}
                className="flex items-center justify-between p-3 rounded-xl hover:bg-zinc-50 cursor-pointer transition-colors group">
            ...
            {orders.length===0 && <p className="text-sm text-zinc-400 text-center py-6">No orders yet</p>}
          </div>
        </div>
      </div>
    </div>
