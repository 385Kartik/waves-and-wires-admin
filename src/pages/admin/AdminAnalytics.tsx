...
export default function AdminAnalytics() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [revRes, topRes, catRes] = await Promise.all([
        supabase.from('admin_revenue_by_day').select('*').order('date', { ascending: true }),
        supabase.from('admin_top_products').select('*'),
        supabase.from('order_items')
          .select('total, products(category_id, categories(name))')
          .gte('created_at', since),
      ]);

      // Fill every day in range (even 0-revenue days)
      ...
      setRevData(interval.map(d => {
        const k = format(d, 'yyyy-MM-dd');
        return revMap[k] ?? { date: k, revenue: 0, order_count: 0 };
      }));
  ...
  const totalRev = revData.reduce((s,d) => s + Number(d.revenue), 0);
  const totalOrd = revData.reduce((s,d) => s + Number(d.order_count), 0);
    ...
    <div className="p-5 sm:p-6 space-y-5 sm:space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-500">Business overview</p>
        </div>
        <div className="flex gap-1.5 bg-white border border-zinc-200 rounded-xl p-1">
          {[['7','7D'],['30','30D'],['90','90D']].map(([v,l]) => (
            <button key={v} onClick={() => setRange(v)}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${range===v?'bg-zinc-900 text-white shadow-sm':'text-zinc-500 hover:text-zinc-800'}`}>
              {l}
            </button>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          [`Revenue (${range}d)`, INR(totalRev), 'From paid & delivered orders'],
          [`Orders (${range}d)`,  totalOrd.toString(), 'Non-cancelled orders'],
          ['Avg Order Value',     INR(Math.round(avgOrd)), 'Per order'],
        ].map(([label, value, sub]) => (
          <div key={label} className="bg-white rounded-2xl border border-zinc-200/80 p-5">
      ...
      <div className="bg-white rounded-2xl border border-zinc-200/80 p-5 sm:p-6">
        <h2 className="text-sm font-bold text-zinc-900 mb-5">Revenue Over Time</h2>
        {loading
          ? <div className="h-56 rounded-xl bg-zinc-100 animate-pulse"/>
          : totalRev === 0
            ? <div className="h-56 flex items-center justify-center text-sm text-zinc-400">No revenue data for this period</div>
            : (
              <ResponsiveContainer width="100%" height={220}>
                <AreaChart data={revData} margin={{top:0,right:0,left:-20,bottom:0}}>
                  ...
                  <Tooltip contentStyle={{background:'#18181b',border:'1px solid #27272a',borderRadius:12,fontSize:12,color:'#fff'}}
                    formatter={(v:number) => [INR(v), 'Revenue']} labelFormatter={l => format(new Date(l), 'PPP')}/>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 sm:gap-6">
        ...
        <div className="bg-white rounded-2xl border border-zinc-200/80 p-5 sm:p-6">
          ...
          {loading
            ? <div className="space-y-3">{[...Array(5)].map((_,i) => <div key={i} className="h-10 rounded-xl bg-zinc-100 animate-pulse"/>)}</div>
            : topProds.length === 0
                ...
                <div className="space-y-1">
                  {topProds.slice(0, 8).map((p, i) => (
                    <div key={p.id} className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-zinc-50 transition-colors">
                      ...
                      {p.image
                        ? <img src={p.image} alt={p.name} className="h-9 w-9 rounded-xl object-cover border border-zinc-100 shrink-0"/>
              ...
              : (
                <>
                  <ResponsiveContainer width="100%" height={200}>
                    <PieChart>
                      <Pie data={catData} cx="50%" cy="50%" outerRadius={80} innerRadius={35}
