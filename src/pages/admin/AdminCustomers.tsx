...
interface CustomerOrder { id:string; order_number:string; status:string; total:number; created_at:string; }
const INR=(n:number)=>`₹${n.toLocaleString('en-IN')}`;
const S_STYLE:Record<string,string>={delivered:'bg-emerald-50 text-emerald-700',cancelled:'bg-red-50 text-red-600',pending:'bg-amber-50 text-amber-700'};
const PAGE=20;
...
export default function AdminCustomers() {
  ...
  const load=useCallback(async()=>{
    setLoading(true);
    let q=supabase.from('profiles').select('*',{count:'exact'}).eq('is_admin',false).order('created_at',{ascending:false}).range(page*PAGE,(page+1)*PAGE-1);
    if(search.trim())q=q.ilike('full_name',`%${search.trim()}%`);
    const {data,count}=await q;
    const ids=(data??[]).map((c:any)=>c.id);
    let statsMap:Record<string,{count:number;total:number}>={};
    if(ids.length){
      const {data:stats}=await supabase.from('orders').select('user_id,total').in('user_id',ids).eq('payment_status','paid');
      (stats??[]).forEach((o:any)=>{
        if(!statsMap[o.user_id])statsMap[o.user_id]={count:0,total:0};
        statsMap[o.user_id].count+=1;statsMap[o.user_id].total+=Number(o.total);
      });
    }
    setCustomers((data??[]).map((c:any)=>({...c,order_count:statsMap[c.id]?.count??0,total_spent:statsMap[c.id]?.total??0})));
  ...
  async function viewCustomer(c:Customer){
    setSelected(c);
    const {data}=await supabase.from('orders').select('id,order_number,status,total,created_at').eq('user_id',c.id).order('created_at',{ascending:false}).limit(10);
    setOrders(data??[]);
  }
  ...
  return(
    <div className="p-6 space-y-5">
      <div><h1 className="text-xl font-bold text-zinc-900">Customers</h1><p className="text-sm text-zinc-500">{total} registered</p></div>

      <div className="bg-white rounded-2xl border border-zinc-200/80 p-4">
        <div className="relative">
          <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-zinc-400"/>
          <input value={search} onChange={e=>{setSearch(e.target.value);setPage(0);}} placeholder="Search by name…"
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-zinc-50">
              {loading?[...Array(6)].map((_,i)=><tr key={i}>{[...Array(6)].map((_,j)=><td key={j} className="px-4 py-4"><div className="h-4 rounded-lg bg-zinc-100 animate-pulse"/></td>)}</tr>)
              :customers.map(c=>(
                <tr key={c.id} className="hover:bg-zinc-50/70 transition-colors group">
                  <td className="px-4 py-3.5">
                    <div className="flex items-center gap-3">
                      <div className="h-9 w-9 rounded-full bg-gradient-to-br from-orange-400 to-orange-600 flex items-center justify-center text-white font-bold text-sm shrink-0">
                        {(c.full_name??c.email??'G').charAt(0).toUpperCase()}
                      </div>
                      <div>
                  ...
                  <td className="px-4 py-3.5 text-right">
                    <button onClick={()=>viewCustomer(c)} className="p-1.5 rounded-lg text-zinc-400 hover:text-zinc-700 hover:bg-zinc-100 transition-all opacity-0 group-hover:opacity-100">
                      <Eye className="h-4 w-4"/>
                    </button>
                  </td>
                </tr>
              ))}
              {!loading&&customers.length===0&&<tr><td colSpan={6} className="px-4 py-12 text-center text-zinc-400 text-sm">No customers found</td></tr>}
            </tbody>
          </table>
        </div>
            ...
            <div className="flex-1 p-6 space-y-5">
              <div className="flex items-center gap-4">
                <div className="h-14 w-14 rounded-2xl bg-gradient-to-br from-orange-400 to-orange-600 flex items-center justify-center text-white font-black text-xl shadow-md shadow-orange-200">
                  {(selected.full_name??selected.email??'G').charAt(0).toUpperCase()}
                </div>
                <div>
